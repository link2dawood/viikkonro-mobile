...
class MonthGrid extends StatelessWidget {
  const MonthGrid({super.key, required this.year, required this.month, required this.repository, required this.today, this.compact = false});
  ...
  double _cellHeight(BuildContext context) => (compact ? 36.0 : 46.0) * MediaQuery.textScalerOf(context).scale(14) / 14;
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      children: [
        Row(
          children: [
            SizedBox(width: 32, child: Mono(context.s.week, size: compact ? 9 : 10)),
            for (var i = 0; i < 7; i++)
              Expanded(
                child: Center(child: Mono(context.weekdayShort(DateTime(2026, 9, 14 + i)), size: compact ? 10 : 11)),
              ),
          ],
        ),
        const SizedBox(height: 8),
        for (var row = 0; row < rows; row++) _row(context, addCalendarDays(start, row * 7)),
      ],
    );
  }
        ...
        SizedBox(
          ...
          child: Semantics(
            ...
            child: ExcludeSemantics(
              child: InkWell(
                onTap: () => Navigator.pushNamed(context, '/viikko-$week-$weekYear'),
                child: SizedBox(
                  height: _cellHeight(context),
                  child: Center(child: Mono('$week', size: 12, weight: FontWeight.w600)),
                ),
              ),
            ),
          ),
        ),
        for (var day = 0; day < 7; day++) Expanded(child: _day(context, addCalendarDays(monday, day))),
      ],
    );
  }
  ...
  Widget _day(BuildContext context, DateTime date) {
    ...
    final current = date == dateOnly(today) && inMonth; // FP-M03: current month only.
    final events = repository.on(date);
    final holiday = events.any((e) => !e.flag);
    final flag = events.any((e) => e.flag);
    final label = [context.headingDate(date), if (holiday || flag) ...events.map((e) => e.name)].join(', ');
    return Semantics(
      label: label,
      button: true,
      selected: current,
      child: ExcludeSemantics(
        child: InkWell(
          ...
          onTap: () => inMonth ? Navigator.pushNamed(context, '/viikonpaiva?paiva=${formatDate(date)}') : Navigator.pushNamed(context, '/kuukausi-${date.month}-${date.year}'),
          child: Container(
            height: _cellHeight(context),
            margin: const EdgeInsets.all(1),
            decoration: BoxDecoration(
              borderRadius: BorderRadius.circular(9),
              color: holiday && inMonth ? context.colors.secondary.withValues(alpha: .13) : null,
              // FP-M03: today is ringed, not filled, so its date stays readable.
              border: current ? Border.all(color: context.colors.primary, width: 2) : null,
            ),
                ...
                children: [
                  Text(
                    ...
                    style: TextStyle(
                      ...
                      fontWeight: current || holiday ? FontWeight.w700 : FontWeight.w400,
                      color: inMonth ? context.colors.onSurface : context.colors.onSurfaceVariant.withValues(alpha: .42),
                    ),
                  ),
