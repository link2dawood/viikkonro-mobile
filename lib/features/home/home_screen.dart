...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Widget build(BuildContext context) {
    final now = dateOnly(widget.today),
        week = isoWeek(now),
        year = isoYear(now),
        s = context.s;
      ...
      children: [
        Row(
          children: [
            ...
            Flexible(
              child: Mono(s.eyebrow, size: 11, color: context.colors.primary),
            ),
          ],
        ),
        const SizedBox(height: 14),
        PageHeading(s.homeTitle, subtitle: s.homeLead),
        SitePanel(
          child: LayoutBuilder(
            builder: (context, c) {
              final headline = Column(
                ...
                children: [
                  ...
                  Semantics(
                    ...
                    child: ExcludeSemantics(
                      child: InkWell(
                        ...
                        onTap: () =>
                            Navigator.pushNamed(context, '/vuosi-$year'),
                        child: FittedBox(
                  ...
                  Mono(context.range(monday, sunday)),
        ...
        Wrap(
          ...
          children: [
            for (var delta = -3; delta <= 3; delta++)
              _weekChip(context, addCalendarDays(now, delta * 7), delta == 0),
          ],
        ),
        // FP-H11 and FP-X01: the week, its span, and the matching site page.
        ResultActions(
          text: '${s.weekLabel(week)}/$year (${context.range(monday, sunday)})',
        ...
        SitePanel(
          child: Column(
            ...
            children: [
              ...
              Text(
                context.range(
                  mondayOf(isoWeek(selected), isoYear(selected)),
                  sundayOf(isoWeek(selected), isoYear(selected)),
                ),
              ),
              TextButton.icon(
                onPressed: () => Navigator.pushNamed(
                  context,
                  '/viikko-${isoWeek(selected)}-${isoYear(selected)}',
                ),
        ...
        TextButton(
          onPressed: () => Navigator.pushNamed(
            context,
            '/kuukausi-${now.month}-${now.year}',
          ),
          child: Text(s.thisMonth),
        ),
        if (next != null) ...[
          ...
          ListTile(
            ...
            subtitle: Wrap(
              ...
              children: [
                ...
                Mono(
                  daysBetween(now, next.date) == 0
                      ? s.today
                      : s.dayCount(daysBetween(now, next.date)),
                  size: 12,
            ...
            onTap: () =>
                Navigator.pushNamed(context, '/pyhapaivat-${next.date.year}'),
          ),
        ],
        ...
        SectionTitle(s.info),
        Text(s.isoExplanation),
        TextButton(
          onPressed: () => Navigator.pushNamed(context, '/ukk'),
          child: Text(s.faq),
        ),
      ],
    );
  ...
  Widget _weekChip(BuildContext context, DateTime date, bool current) =>
      ChoiceChip(
        selected: current,
        label: Text(
          context.s.weekShort(isoWeek(date)),
          style: const TextStyle(fontFamily: 'PlexMono', fontSize: 12),
        ),
        onSelected: (_) => Navigator.pushNamed(
          context,
          '/viikko-${isoWeek(date)}-${isoYear(date)}',
        ),
        showCheckmark: false,
      );
}
